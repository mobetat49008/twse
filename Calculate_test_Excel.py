...
def Get_change(stock_list, req_item):
    ...
    result = Queue()
    Pc.stock_change_crawler(stock_list, req_item, result)
    change = result.get()
    '''
    for i in range(len(stock_list)//urlsplitlength +1):
        small_stock_list = stock_list[i*urlsplitlength:min(len(stock_list),(i+1)*urlsplitlength)]
        Pc.stock_change_crawler(small_stock_list, req_item, result)
...
def process(stock_list, weight, req_item):
    ...
    if abs(last_PreIndex - PreIndex)>1:        
        t = threading.Thread(target=Line_test.lineNotifyMessage, args=(token, msg))
        last_PreIndex = PreIndex
    else:
        t = threading.Thread(target=Line_test.lineNotifyMessage, args=(token2, msg))
    t.start()
...
EveryDay_Update_Flag = 0
def EveryDay_Update_Flag_Change():
    global EveryDay_Update_Flag
    EveryDay_Update_Flag = 1
    print('Update_Flag_UP')
    return
...
def Process_start():
    global Process_Flag
    Process_Flag = 1
    print('Process_Flag_UP')
    return
    
def Process_stop():
    global Process_Flag
    Process_Flag = 0
    print('Process_Flag_DOWN')
    return
...
if __name__ == '__main__': 
    ...
    scheduler.add_job(EveryDay_Update_Flag_Change, trigger='cron', day_of_week='mon-fri', hour='21', minute="00", second="0",id='my_job_id_1',misfire_grace_time=30)
    scheduler.add_job(Process_start, trigger='cron', day_of_week='mon-fri', hour='08', minute="35", second="0",id='my_job_id_2',misfire_grace_time=30)
    scheduler.add_job(Process_start, trigger='cron', day_of_week='mon-fri', hour='13', minute="25", second="10",id='my_job_id_3',misfire_grace_time=30)
    scheduler.add_job(Process_stop, trigger='cron', day_of_week='mon-fri', hour='09', minute="01", second="0",id='my_job_id_4',misfire_grace_time=30)
    scheduler.add_job(Process_stop, trigger='cron', day_of_week='mon-fri', hour='13', minute="31", second="30",id='my_job_id_5',misfire_grace_time=30)
